...
export async function mongoQueryMultiple(queryArray, mongoCollection) {
  const client = new MongoClient(uri);
  let queryResult = "Error in query";
  try {
    const database = client.db(mongoDatabase);
    ...
    for (const i in queryArray) {
      Object.assign(queryObject, {
        [queryArray[i][0]]: { $gte: queryArray[i][1], $lte: queryArray[i][2] },
      });
    }
...
export async function mongoInsertOne(input, mongoCollection) {
  const client = new MongoClient(uri);
